...
getStart = function(link)
{
  link %>% read_html() %>% 
    html_nodes("div.wd-ui-cont p") %>% 
    html_text() %>% 
    str_extract("펀딩기간.+") %>% 
...
getEncore = function(link)
{
  link %>% read_html() %>% 
    html_nodes(".project-state-info p span") %>% 
    html_text() %>% 
    unique() %>% 
...
getFundinginfoUrl = function(link)
{
